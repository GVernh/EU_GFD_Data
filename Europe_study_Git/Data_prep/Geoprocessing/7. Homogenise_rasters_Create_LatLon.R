...
setwd("./") # Specify where computed geodiversity layers are located
...
# Climate data accessed: https://chelsa-climate.org/

setwd("D:/Europe_study_data/")

FD_rasters = list.files(path = "./Functional_diversity/", # Specify directory where functional diversity rasters are located
                        pattern =".tif$",
                        full.names = T,
                        recursive = F)

LU = list.files(path= "./Land_use/", # Specify directory where land use rasters are located
                pattern = ".tif$",
                recursive = F,
                full.names = T)

clim_rast = list.files(path = "./Climate/",# Specify directory where climate rasters are located
                          pattern = ".tif$",
                          recursive = F,
                          full.names = T)
...
rast = terra::rast("./EU_landSystem.tif")
...
terra::writeRaster(rast, filename = "./Finalised_rasters/EU_Landsystem_by_names.tif", 
...
terra::writeRaster(rlon, filename = "./Finalised_rasters/longitude.tif", gdal = "TFW=YES",
                   overwrite = TRUE)
terra::writeRaster(rlat, filename = "./Finalised_rasters/latitude.tif", gdal = "TFW=YES",
                   overwrite = TRUE)
