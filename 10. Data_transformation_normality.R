...
# --------------- DATA -------------------------
setwd("D:/Europe_study_data/Analysis/Finalised_dataset/")
...
# ----------------- All plot transformation ------------------------
rm(list = ls())
setwd("D:/Europe_study_data/Analysis/Finalised_dataset/")


data_all = read.csv("./Full_dataset_all_plots.csv")
data_all <- data_all %>%
  select("Land_system_names","Land_use", "lon", "lat","Landform_sha", everything())
trans_data = data_all

for(i in seq(6,length(trans_data))){
  x = skewness.test(trans_data[[i]])
  if (x$statistic > 0.6 | x$statistic < -1.25) {c = trans_data[[i]]^(1/3)  # c = log10(as.data.frame(data_5x5[[i]] +1))
  s = print(colnames(trans_data[i]))
  trans_data[paste(s)] = c
...
setwd("D:/Europe_study_data/Analysis/Finalised_dataset/Transformed/")
write.csv(x = trans_data, file = "./Full_dataset_all_plots_transformed.csv", row.names = F)


# ----------------- 1m plot transformation ------------------------
rm(list = ls())
setwd("D:/Europe_study_data/Analysis/Finalised_dataset/")


data_all = read.csv("./Full_dataset_1m_plots.csv")
data_all <- data_all %>%
  select("Land_system_names","Land_use", "lon", "lat","Landform_sha", everything())
trans_data = data_all
...
setwd("D:/Europe_study_data/Analysis/Finalised_dataset/Transformed/")
write.csv(x = trans_data, file = "./Full_dataset_1m_plots_transformed.csv", row.names = F)