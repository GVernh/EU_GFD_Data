...
boundary = sf::st_read("./Raw data/Boundary_polygon.shp")
ext = ext(boundary) # Extent of interest
Proj_rast = terra::rast(res = 250, crs = "EPSG:3035", ext = ext)

# ------------- AVERAGE SOIL DATA ------------
soil_0_5 = list.files(path = "./Raw data/Soil/Europe/Selected_soil_data/0_5/",
                      pattern = ".tif$",
                      recursive = F,
                      full.name = T)
soil_5_15 = list.files(path = "./Raw data/Soil/Europe/Selected_soil_data/5_15/",
                      pattern = ".tif$",
                      recursive = F,
                      full.name = T)
...
foreach(i = seq_along(1:length(soil_0_5)), .packages= "terra") %dopar% {
  ...
  names(soil_250m) = x
  path= paste0("./Projected_rasters/Geodiversity/", substr(x, 1, 6) , "_250m.tif")
...
# ----------------- REPROJ DEM -------------------
DEM = terra::rast("./Raw data/DEM/Europe/DEM_25/DEM_25.tif")
...
terra::writeRaster(x = DEM_250m, filename = "./Projected_rasters/Geodiversity/DEM_250m.tif", gdal = "TFW=YES", 
                   overwrite = T)
# terra::writeRaster(x = DEM_125m, filename = "./Projected_rasters/Geodiversity/DEM_125m.tif", gdal = "TFW=YES", 
    #                 overwrite = T)
rm(DEM, DEM_250m)
# rm(DEM_125)
