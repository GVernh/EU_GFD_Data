...
setwd("D:/Europe_study_data/Writeup/Round_2_content/Spatially_corrected_data/")
...
left = small_multiple(x_left,
               dot_args = list(aes(shape = dimension, col = dimension), size = 0.7),
               whisker_args = list(aes(linetype = dimension),
                                   shape = shape, color = shape)
               ) +
  theme_bw(base_size = 20) + ylab("Standardised coefficient Estimate") +
...
right = small_multiple(x_right,
               dot_args = list(aes(shape = dimension, col = dimension)),
               whisker_args = list(aes(linetype = dimension))
) +
